...
# from Jana.modules.sql import warns_sql as warnssql
import Jana.modules.sql.blacklist_sql as blacklistsql

# from Jana.modules.sql import cust_filters_sql as filtersql
# import Jana.modules.sql.welcome_sql as welcsql
import Jana.modules.sql.locks_sql as locksql
import Jana.modules.sql.notes_sql as sql

# from Jana.modules.rules import get_rules
import Jana.modules.sql.rules_sql as rulessql
from Jana import EVENT_LOGS,OWNER_ID, SUPPORT_CHAT, dispatcher
from Jana.__main__ import DATA_IMPORT
from Jana.modules.connection import connected
from Jana.modules.helper_funcs.alternate import typing_action
from Jana.modules.helper_funcs.chat_status import user_admin
from Jana.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("Jana{}backup.json".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Jana{}backup.json".format(chat_id), "rb"),
        caption="📤*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Jana-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("Jana{}backup.json".format(chat_id))  # Cleaning file
